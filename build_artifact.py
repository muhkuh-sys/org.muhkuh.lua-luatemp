...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-tempsensor-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-tempsensor-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.1-tempsensor-*.xml".')
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-tempsensor-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-tempsensor-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-tempsensor-*.xml".')
